...
class LinearSpline(ABC, nn.Module):
    ...
    def get_clip_equivalent(self):
        ...
        grid_tensor = torch.linspace(
            self.x_min.item(), self.x_max.item(), self.num_knots, device=self.device
        ).expand((self.num_activations, self.num_knots))
...
class clip_activation(nn.Module):
    ...
    def forward(self, x):
        return (
            self.slopes * (torch.relu(x - self.x1) - torch.relu(x - self.x2)) + self.y1
        )
    ...
    def integrate(self, x):
        return (
            ...
            / 2
            * (
                (torch.relu(x - self.x1) ** 2 - torch.relu(x - self.x2) ** 2)
                + self.y1 * x
            )
        )
