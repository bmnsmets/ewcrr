...
def accelerated_gd_batch(
    x_noisy,
    model,
    sigma: Tensor,
    ada_restart: bool = False,
    stop_condition=None,
    lmbd=1,
    grad_op=None,
    **kwargs,
    ...
    t = torch.ones(x.shape[0], device=x.device).view(-1, *(1,) * (x.ndim - 1))
    ...
    for i in range(max_iter):
        ...
        if grad_op is None:
            ...
            grad = model.grad_denoising(
                z[idx], x_noisy[idx], sigma=sigma[idx], cache_wx=False, lmbd=lmbd
            )
        else:
            grad = grad_op(z[idx])
        ...
        if i > 0:
            res[idx] = torch.norm(x[idx] - x_old[idx], p=2, dim=(1, 2, 3)) / (
                torch.norm(x[idx], p=2, dim=(1, 2, 3))
            )
...
def accelerated_gd_single(
    ...
    **kwargs,
    ...
    for i in range(max_iter):
        ...
        if grad_op is None:
            grad = model.grad_denoising(
                z, x_noisy, sigma=sigma, cache_wx=False, lmbd=lmbd
            )
        else:
            grad = grad_op(z)
...
def build_model(config):
    ...
    if len(param_conv_layer["kwargs"]["num_channels"]) != (
        len(param_conv_layer["kwargs"]["size_kernels"]) + 1
    ):
        raise ValueError(
