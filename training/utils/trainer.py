...
from training.utils.utils import build_model, batch_SSIM, batch_PSNR
...
class Trainer:
    ...
    def __init__(self, config, seed, device):
        ...
        # Set the DEQ solver
        self.denoise = deep_equilibrium.DEQFixedPoint(
            self.model,
            self.config["training_options"]["fixed_point_solver_fw_params"],
            self.config["training_options"]["fixed_point_solver_bw_params"],
        )
    ...
    def set_optimization(self):
        ...
        params_activations_coeff = [
            model.activation_cvx.coefficients,
            model.activation_ccv.coefficients,
        ]
        params_dicts.append(
            {"params": params_activations_coeff, "lr": lr["spline_activation"]}
        )
        ...
        # 4 - spline scaling
        params_dicts.append(
            {"params": [model.spline_scaling.coefficients], "lr": lr["spline_scaling"]}
        )
        ...
        if self.config["training_options"]["scheduler"]["use"]:
            self.scheduler = torch.optim.lr_scheduler.StepLR(
                self.optimizer,
                step_size=1,
                gamma=self.config["training_options"]["scheduler"]["gamma"],
                # verbose=True,
            )
    ...
    def train_epoch(self):
        ...
        for batch_idx, data in enumerate(tbar):
            ...
            if (
                self.config["training_options"]["scheduler"]["use"]
                and self.batch_seen
                % self.config["training_options"]["scheduler"]["n_batch"]
                == 0
            ):
                # check n steps
                nsteps = (
    ...
    def valid_epoch(self):
        ...
        with torch.no_grad():
            ...
            for batch_idx, data in enumerate(tbar_val):
                ...
                out_val = torch.clamp(output, 0.0, 1.0)

                loss_val = loss_val + loss.cpu().item()

                psnr_val = psnr_val + batch_PSNR(out_val, data, 1.0)
                ssim_val = ssim_val + batch_SSIM(out_val, data, 1.0)
        ...
        tbar_val.set_description(
            "EVAL ({}) | MSELoss: {:.5f} |PSNR:{:.4f}".format(
                self.valid_epoch_num, loss_val, psnr_val
            )
        )
        ...
        print(
            "EVAL ({}) | MSELoss: {:.5f} |PSNR:{:.4f}".format(epoch, loss_val, psnr_val)
        )
    ...
    def save_checkpoint(self, epoch):
        ...
        self.checkpoint_dir = os.path.join(
            self.config["logging_info"]["log_dir"],
